...
\begin{abstract}
  ...
  and more accessible way. Although these provide significantly
  simpler way of reading and presenting quantum computations, manual
  manipulation of such graphs is slow, arduous, and error prone. We
  ...
  that meets the underlying categorical semantics for the model of
  quantum computation. In particular, we present system, which has
  been implemented, with a fixed logical kernel and which gives a
  declarative account and supports derived rules. Soundness of the
  system is proved with respect to the underlying model of quantum
  computation and completeness is an open problem.
