...
% concerning:
%    symbolic computation, automated reasoning, formalised mathematics.
\end{abstract}
...
the preceding section thus provides a very expressive notation quantum
processes.
...
use colours to denote two families:
\begin{gather*}
...
operator.\footnote{Uniform copying operations are forbidden by the
  no-cloning theorem \cite{no-cloning}, but such operations are
  ...
  remarks hold true for erasing.}  Notice that $\delta_Z$ has one edge
...
\begin{remark}
  ...
  Their graphical notation is dual to ours, with the vertices
  representing qubits and the edges representing entanglement between
  the individual quibits.  Within the formalism of this paper, a state
  is simply a process with no inputs; that is, a concrete graph with
  empty domain.  An important application is to show that states or
  computations implemented differently are equivalent: this includes
  graph states, and measurement based computations more generally.
\end{remark} 
...
Consider the equations from Figure \ref{fig:graph-quant-eqns} which
involve only one colour: these allow the remarkable \emph{spider
  theorem}, first noted in \cite{Coecke2006Quantum-Measure}, to be
proved:
...
\inlinegraphic{1.3em}{compact4} $\qquad\quad$ 
\inlinegraphic{2.5em}{compact5} $=$
\inlinegraphic{2.5em}{compact6}
\end{center}

\noindent However, composition with semi-circles ($d_Q$ and $e_Q$ from
...
\emph{matching}, which captures the intuitive idea of a graph with
variable nodes occurring within another graph:

\begin{definition}
  A variable-node (resp., concrete) graph $G$ \emph{matches} another
  graph $H$ if there exists an open embedding $G\to H$ which is strict
  on the non-variable (resp., non-boundary) nodes.  This open
  embedding is called a \emph{matching}. The notation $G \leq_v H$ is
  used for $G$ matches $H$.
\end{definition}
...
% variable node is instantiated to some other node, its type will have
...
%represented with !-boxes in Figure~\ref{fig:spider}.
...
matching $G$ is defined as the set of graphs closed by the following
operations on $G$:
...
additional data, or have an \emph{angle} and are either the $X$ or $Z$
