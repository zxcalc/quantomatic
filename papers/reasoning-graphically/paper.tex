...
\begin{definition}
  A variable-node (resp., concrete) graph $G$ \emph{matches} another graph
  $H$ if there exists an open embedding $G\to H$ which is strict on
  the non-variable (resp., non-boundary) nodes.  This open embedding
  is called a \emph{matching}.
\end{definition}
