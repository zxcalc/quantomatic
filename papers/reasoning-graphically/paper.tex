...
\item[Arrows:] two families of arrows $X$ and $Z$:
  \begin{align*}
...
Theorem in Figure~\ref{fig:informal-spider-law} for instance is
...
\begin{figure}[t]
$$\begin{array}{ccc}
\inlinegraphic{8em}{spider_lhs_patt} & \;=\; & \inlinegraphic{6em}{spider_rhs_patt}
\end{array}$$
\label{fig:spider-thm-patt}\caption{The Spider Theorem expressed using graph patterns.}
\end{figure}
...
*** lifting concrete laws to graph-pattern laws ***

\subsection{Adequacy} 

An equation on graph patterns corresponds to
a possibly infinite set of equations betweens concrete graphs. We
might expect that such equations express too much: they may allow
rewrites which are not intended. We prove \emph{adequacy}, which
states that this is not the case. In particular, given an equation
between concrete graphs, $G = H$, the equations between the lifting of
these to variable node graphs by replacing boundary nodes with
variable nodes is a conservative extension. The proof follows directly
from the obervation that every instance of the lifted variable node
equation corresponds to compsition of $G$ and $H$ with some other
graph (the non-embedded part).
...
A new graph can be introduced using the reflexivity law. This can be
the left hand side of a conjecture, or a graph that needs to be
computed. By applying the \emph{subst} rule, intermediate results are
derived which can themselves be used to rewrite other rules and
conjetcures. In this way, the system allows derived rules to provide
an abbreviation for a combination of steps. 
