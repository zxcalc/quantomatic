...
\begin{align*}
  ...
  \rTo^{e_A \otimes \id{A}} I \otimes A \iso A
\end{align*}
...
graphs. The chosen model provides the \emph{object-level} graph
formalism. This includes a particular representation for data at the
nodes and edges as well as node-matching and edge-matching
algorithms. The object-level definition also defines a set of
axiomatic equational rules. These are derived 

\subsubsection{Soundness.} An object-level graph system is sound if
their is some equation which can not be derived. 

 The soundness
of a set of axioms in our framework

 framework
requres the model is an instance of a compact closed category, and
that the

For the model we use of qantum computation, this is finite
dimensional hilbert spaces. Because the basic equational rules come
from a model and have been proved for that model, the resulting
preserve the soundness of our graphical system.



 and the families of them represneted by graph
patterns.

An object graph formalsism uses the
...
\subsection{Node in Quantum Computations}
