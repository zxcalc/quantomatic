...
The formalism we have presented for graph patterns provides a
\emph{meta-level} framework for reasoning in some model of concrete
graphs. 

An \emph{object-level} graph formalism is defined by some
model of a compact closed category. 

This includes a particular
representation for data at the nodes and edges as well as
node-matching and edge-matching algorithms. The object-level
definition also defines a set of axiomatic equational rules.

Such a theorem prover starts with a system of equations between
graphs. These are not true of the graphs abstractly, but are proved of
the particular model of concrete graphs being considered.
