...
category.  We present an overview of this structure:  the details
omitted here can be found in \cite{Duncan:thesis:2006}.  

A \emph{concrete graph} $\Gamma$ is 5-tuple $(G, \dom\Gamma, \cod\Gamma,
<_{\text{in}(\cdot)}, <_{\text{out}(\cdot)})$ where:
\begin{itemize}
\item $G = (V,E,C,s,t)$ is a graph;
\item $\dom\Gamma$ and $\cod\Gamma$ are totally ordered disjoint of
...
$\cod \Gamma$ and $\dom \Gamma$ are \emph{signed sets}.  Concrete
graphs form a category whose objects are 


can be seen as giving a signing to each element of the ordinal
one, we can view these sets as maps from the ordinal

can be viewed as a maps from an
...
section we describe an extension to the representation that captures
an infinite family of graphs as a single finite graph. The extension,
which we call \ref{graph patterns}, adds two extra kinds of nodes to
the graph. Namely, {\em banged boxes} which informally represent an
...
which informally represent some, as yet unkown, node in a graph. We
also introduce a notion to replace graph composition, called graph
plugging that allows graph patterns to put together to form larger
graph patterns. 

%Notably, this
%allows us to represent laws about quantum computation, such as the
%Spider Law. Furthermore, this representation is still compositional in
%the sense that we can plug graphs together.
...
node is that it can be replaced by any subgraph of the right
type. Thus, unlike nodes in conrete a graph, variable nodes do not
define an ordering on the edges that are incident. Instead, the edges
that touch a variable node are simply a set. The type of a variable
nodes is thus defined as a multiset rather than a tensor. Any tensor
type can be lifted to a multiset type in the obvious manner, which we
denote by the operation {\tt mset\_of\_tensor}. For example, {\tt
  mset\_of\_tensor($a \tensor b \tensor a$) = $\{(a,2), (b,1)\}$}. We
say that a tensor-type, $T$, is an instance of a multiset type, $M$,
when it's lifting into a multiset is a super-set of $M$: $ M \subseteq
...
now define the semantics for !-boxed graphs, then show how to extend
an the embedding function which corresponds to matching as well as the
plugging operation for !-boxes.

\subsubsection{!-Box Semantics.}
...
\noindent The semantics of a !-boxed graph is then defined as those
members of the set closed by these operation that do have any
!-boxes. Observe that an instance of a !-box graph is defined by
pairing each !-box with the natural number that defines how many
copies are made of it. Thus the $\binterp{G}$ is isomorphic to the set
of $k$-tuples of natural numbers, where $k$ is the number of !-boxes.

\subsubsection{!-Box Matching.}

Matching for !-box graphs is defined in terms of an underlying
matching operation. A !-box graph $G$ matches another one, $H$, iff
$\binterp{G} \supseteq \binterp{H}$. Matching can be given a more
constructive flavour by extending the open-embeddings of variable-node
graphs to !-boxes. 

There is a !-open embedding of $G$ into $H$ if  


\subsubsection{!-Box Plugging.}
