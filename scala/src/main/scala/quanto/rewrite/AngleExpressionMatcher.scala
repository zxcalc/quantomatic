...
class AngleExpressionMatcher(val pVars : Vector[String], val tVars : Vector[String], val mat : RationalMatrix) {
  ...
  def toMap : Map[String, AngleExpression] = if (mat.numCols == 0) Map() else {
    var mp = Map[String,AngleExpression]()
    mat.rows.foldLeft(Map[String,AngleExpression]()) { (mp, row) =>
      val p = RationalMatrix.findPivot(row)
      var coeffs = Map[String,Rational]()
      for (i <- p+1 to mat.line-1)
...
object AngleExpressionMatcher {
  def apply(pVars : Vector[String], tVars : Vector[String]) : AngleExpressionMatcher =
