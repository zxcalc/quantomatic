  ...
  def apply[A](items: Iterable[(A,(Double,Double))]): QuadTree[A] = {
    items.headOption match {
      case Some((_,(hx,hy))) =>
        ...
        items.foldLeft(emptyTree) { case (tr,(v,p)) => tr.insert(v,p) }
  ...
  p: (Double,Double) = (0,0)) extends QuadTree[A]
{
  def insert(v1: A, p1: (Double,Double)) =
    value match {
      case None => copy(value = Some(v1), p = p1)
      case Some(v) =>
        if (abs(p._1 - p1._1) < 0.01 && abs(p._2 - p1._2) < 0.01) {
          ...
          QuadNode[A](x1,y1,x2,y2,Some(v),p).insert(v1,p1)
        } else {
          // otherwise place them on their own leaves
          QuadNode[A](x1,y1,x2,y2).insert(v,p).insert(v1,p1)
        }
    }
