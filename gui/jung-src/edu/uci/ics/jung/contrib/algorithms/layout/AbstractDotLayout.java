...
public abstract class AbstractDotLayout<V, E> extends AbstractLayout<V, E> implements DynamicBoundsLayout {
	 ...
	 *	 
	 * This must be unique (within the graph) for each vertex, and must always
	 * return the same value for any given vertex between a call to
	 * beginLayout() and the corresponding call to endLayout().
	 *	 
	...
	protected double vertexWidth(V v) {
		return 14;
	}

	protected double vertexHeight(V v) {
		return 14;
	}
	...
	/**
	 * Converts a graph to a DOT string. ALWAYS run in the context of
	 * synchronized(getGraph()) {...}.
	 */
