...
public class AKDotLayout<V, E> extends AbstractLayout<V, E> {
	...
	public static <V, E> DirectedGraph<V, E> getSubgraphWithVertices(DirectedGraph<V, E> gr, Collection<V> verts) {
		...
		for (E e : gr.getIncidentEdges(vert)) {
			if (e != cut) {
				...
				if (!comp.contains(next)) {
					addToComponent(next, gr, comp, cut);
				}
			}
		}
	}
		...
		for (E in : trySortedSet(graph.getInEdges(vertex))) {
			...
			if (inverseVertexTable.containsKey(src)) {
				...
				if (srcId != vertexId
						...
						&& dag.findEdge(vertexId, srcId) == null) { // merge multi-edges, ignore self-loops
		...
		for (E out : trySortedSet(graph.getOutEdges(vertex))) {
			...
			if (inverseVertexTable.containsKey(dest)) {
				...
				if (destId != vertexId
						...
						&& dag.findEdge(destId, vertexId) == null) { // merge multi-edges, ignore self-loops
...
//			}
	...
	private void feasibleTree() {
		...
		Graph<Integer, Integer> gr = dag;
		int root = gr.getVertices().iterator().next();
		int numVerts = gr.getVertexCount();
	...
	private boolean updateCutValues() {
		...
		for (int e : tree.getEdges()) {
			...
			int hd, tl;
			if (comps.get(0).contains(gr.getSource(e))) {
				tl = 0;
				hd = 1;
			} else {
				tl = 1;
	...
	private Ordering initOrdering(boolean direction) {
		...
		for (Integer max : tree.getVertices()) {
			...
			if (adjacent.isEmpty()) { // minimal (or maximal) vertex
				q.add(max);
				Integer v;
				while ((v = q.poll()) != null) {
					if (!ord.contains(v)) {
...
//	}
	...
	private int[] initXCoord() {
		...
		int maxRankWidth = 0;
		for (List<Integer> rank : bestOrdering.lists) {
			if (rank.size() > maxRankWidth) {
				maxRankWidth = rank.size();
			}
		}
...
//			for (int i = 0; i < coords.length; ++i) coords[i] -= minCoord;
	...
	private int xLength(int[] coords) {
		...
		for (int e : virtualGraph.getEdges()) {
			...
			if (s < firstVirtual && d < firstVirtual) {
				omega = OMEGA_RR;
			} else if (s < firstVirtual != d < firstVirtual) {
				omega = OMEGA_VR;
			} else {
				omega = OMEGA_VV;
			}
	...
	static class Ordering {
		...
		private List<Integer> adjPositions(int v, int adjRank) {
			...
			for (int i = 0; i < lists[adjRank].size(); ++i) {
				if (neigh.contains(lists[adjRank].get(i))) {
					pos.add(i);
				}
			}
			return pos;
...
//					cross1 = crossings;
					cross1 = 0;
					if (rank > 0) {
						cross1 += crossingsForRank(rank - 1);
					}
					if (rank < numRanks() - 1) {
...
//						cross2 = crossings;
						cross2 = 0;
						if (rank > 0) {
							cross2 += crossingsForRank(rank - 1);
						}
						if (rank < numRanks() - 1) {
