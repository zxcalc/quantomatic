...
public class DetailedErrorDialog {
	...
	public static void showDetailedMessageDialog(
		...
		JOptionPane.showMessageDialog(parent, contents, title, messageType);
	}
}
...
class CollapsiblePanel extends JPanel {
	...
	public void addCollapseListener(CollapseListener collapseListener) {
		listenerList.add(CollapseListener.class, collapseListener);
	}

	public void removeCollapseListener(CollapseListener collapseListener) {
		listenerList.remove(CollapseListener.class, collapseListener);
	}
	...
	protected void fireCollapsed() {
		Object[] listeners = listenerList.getListenerList();
		for (int i = listeners.length - 2; i >= 0; i -= 2) {
			if (listeners[i] == CollapseListener.class) {
				((CollapseListener) listeners[i + 1]).collapsed();
			}
		}
	}
	...
	protected void fireExpanded() {
		...
		for (int i = listeners.length - 2; i >= 0; i -= 2) {
			if (listeners[i] == CollapseListener.class) {
				((CollapseListener) listeners[i + 1]).expanded();
			}
		}
	}
